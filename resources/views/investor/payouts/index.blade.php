        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-green-600">Rp {{ number_format($payout->profit_amount, 0, ',', '.') }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $payout->payment_date->format('d F Y') }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-center">
                @if ($payout->receipt_path)
                  <button @click="receiptModalOpen = true; receiptImageUrl = '{{ asset('storage/' . $payout->receipt_path) }}'" class="px-3 py-1 text-xs font-medium text-indigo-700 bg-indigo-100 rounded-full hover:bg-indigo-200">
                    Lihat Bukti
                  </button>
                @else
