  ...
  <div class="flex h-screen bg-gray-50">
    ...
    <main class="flex-1 overflow-y-auto p-6 lg:p-8">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start mb-8">
        ...
        <button 
          x-data 
          @click.prevent="$dispatch('open-modal', 'add-project-modal')"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div class="bg-white p-6 rounded-lg shadow-md">
          <p class="text-sm font-medium text-gray-500">Proyek Aktif</p>
          <p class="text-3xl font-bold text-indigo-600 mt-1">{{ $activeProjectsCount }}</p>
        </div>
        <div class="bg-white p-6 rounded-lg shadow-md">
          <p class="text-sm font-medium text-gray-500">Proyek Butuh Dana</p>
          <p class="text-3xl font-bold text-yellow-600 mt-1">{{ $fundingNeededCount }}</p>
        </div>
        <div class="bg-white p-6 rounded-lg shadow-md">
          <p class="text-sm font-medium text-gray-500">Proyek Selesai</p>
          <p class="text-3xl font-bold text-green-600 mt-1">{{ $completedProjectsCount }}</p>
        </div>
      </div>
              ...
              <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">
                    {{ $investedQty }} / {{ $project->quantity }} pcs
                  </div>
                  <div class="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                    <div class="bg-blue-600 h-1.5 rounded-full" style="width: {{ $fundingPercentage }}%"></div>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <div class="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                    <div class="bg-green-600 h-1.5 rounded-full" style="width: {{ $productionPercentage }}%"></div>
                  </div>
                </td>
        ...
        <form method="POST" action="{{ route('admin.projects.store') }}" enctype="multipart/form-data" class="p-6">
          ...
          <div class="mt-6 space-y-4">
            ...
            {{-- =================== INPUT BARU DITAMBAHKAN DI SINI =================== --}}
              <div>
                <x-input-label for="nominal_proyek" :value="__('Nominal Proyek dari Buyer (Rp/pcs)')" />
                <x-text-input id="nominal_proyek" name="nominal_proyek" type="number" step="1000" class="mt-1 block w-full" :value="old('nominal_proyek')"/>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <x-input-error :messages="$errors->get('material_cost')" class="mt-1" />
              </div>
              <div>
              ...
              <div>
                ...
                <x-text-input id="convection_profit" name="convection_profit" type="number" step="50" class="mt-1 block w-full" :value="old('convection_profit')" required />
